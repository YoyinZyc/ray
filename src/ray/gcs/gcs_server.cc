...
void GcsServer::RegisterRpcServices() {
  ...
  // Intentionally NOT leader-gated: RaySyncer is an in-memory resource-view
  // stream (not a request/response handler).
  //  It has no shared-storage writes or leader-only side
  // effects, so it is exempt.
